...
class UserProfile extends HealthProfile implements HealthPlan {
    ...
    public String generateWorkoutPlan() {
        ...
        String line = "  ==========================================";
        sb.append("\n" + Ansi.BOLD + Ansi.BRIGHT_WHITE + line + Ansi.RESET + "\n");
        sb.append(Ansi.BOLD + Ansi.BRIGHT_WHITE + "       PERSONALIZED WORKOUT PLAN          " + Ansi.RESET + "\n");
        sb.append(Ansi.BOLD + Ansi.BRIGHT_WHITE + line + Ansi.RESET + "\n");

        sb.append(Ansi.BOLD + Ansi.CYAN + "  Name   : " + Ansi.BRIGHT_WHITE + name + Ansi.RESET + "\n");
        sb.append(Ansi.BOLD + Ansi.CYAN + "  BMI    : " + bmiColor + String.format("%.1f", bmi)
                + " (" + getBMICategory() + ")" + Ansi.RESET + "\n");
        sb.append(Ansi.BOLD + Ansi.CYAN + "  Mental : " + getMentalColor() + mentalStatus + Ansi.RESET + "\n");
        sb.append(Ansi.BOLD + Ansi.CYAN + "  Goal   : " + Ansi.BRIGHT_YELLOW + goal + Ansi.RESET + "\n");
        sb.append(Ansi.BLUE + "  ------------------------------------------\n" + Ansi.RESET);
        sb.append(Ansi.BOLD + Ansi.UNDERLINE + Ansi.BRIGHT_WHITE + "  WEEKLY SCHEDULE:\n" + Ansi.RESET);
        List<String> schedule = buildSchedule();
        String[] days = { "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday" };
        String[] rowColors = { Ansi.CYAN, Ansi.WHITE };
        for (int i = 0; i < days.length; i++) {
            String activity = (i < schedule.size()) ? schedule.get(i) : "Rest";
            String dayLabel = (days[i].equals("Saturday") || days[i].equals("Sunday"))
                    ? Ansi.MAGENTA + days[i]
                    : Ansi.YELLOW + days[i];
            sb.append("  " + Ansi.BOLD + dayLabel + Ansi.RESET
                    + Ansi.BLUE + " -> " + Ansi.RESET
                    + rowColors[i % 2] + activity + Ansi.RESET + "\n");
        }
    ...
    public String getAdvice() {
        StringBuilder sb = new StringBuilder();
        sb.append("\n" + Ansi.BOLD + Ansi.BRIGHT_MAGENTA
                + "  ----------- HEALTH ADVICE -----------  " + Ansi.RESET + "\n");
        switch (mentalStatus) {
            case DEPRESSED:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_RED    + "  [Mental]  " + Ansi.RESET
                        + Ansi.RED + "You are not alone. Start with gentle movement.\n" + Ansi.RESET);
                sb.append(Ansi.RED + "            Consider speaking with a professional if needed.\n" + Ansi.RESET);
                break;
            case STRESSED:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_YELLOW + "  [Mental]  " + Ansi.RESET
                        + Ansi.YELLOW + "Take deep breaths. Exercise at your own pace.\n" + Ansi.RESET);
                sb.append(Ansi.YELLOW + "            Prioritize sleep and reduce caffeine.\n" + Ansi.RESET);
                break;
            case NORMAL:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_GREEN  + "  [Mental]  " + Ansi.RESET
                        + Ansi.GREEN + "Great mental state! Stay consistent and motivated.\n" + Ansi.RESET);
                break;
        }
        ...
        switch (getBMICategory()) {
            case "Underweight":
                sb.append(Ansi.BOLD + Ansi.BRIGHT_CYAN   + "  [BMI]     " + Ansi.RESET
                        + Ansi.CYAN + "Eat nutritious, calorie-dense foods. Avoid skipping meals.\n" + Ansi.RESET);
                break;
            case "Normal":
                sb.append(Ansi.BOLD + Ansi.BRIGHT_GREEN  + "  [BMI]     " + Ansi.RESET
                        + Ansi.GREEN + "BMI is healthy. Maintain balance between diet and exercise.\n" + Ansi.RESET);
                break;
            case "Overweight":
                sb.append(Ansi.BOLD + Ansi.BRIGHT_YELLOW + "  [BMI]     " + Ansi.RESET
                        + Ansi.YELLOW + "Reduce processed foods. Aim for 7,000+ steps daily.\n" + Ansi.RESET);
                break;
            case "Obese":
                sb.append(Ansi.BOLD + Ansi.BRIGHT_RED    + "  [BMI]     " + Ansi.RESET
                        + Ansi.RED + "Consult a doctor before starting. Low-impact exercise is best.\n" + Ansi.RESET);
                break;
        }
        ...
        switch (goal) {
            case LOSE_WEIGHT:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_MAGENTA + "  [Goal]    " + Ansi.RESET
                        + Ansi.MAGENTA + "Create a modest calorie deficit. Stay hydrated.\n" + Ansi.RESET);
                break;
            case BUILD_MUSCLE:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_MAGENTA + "  [Goal]    " + Ansi.RESET
                        + Ansi.MAGENTA + "Eat enough protein (1.6-2.2g per kg body weight).\n" + Ansi.RESET);
                break;
            case MAINTAIN_WEIGHT:
                sb.append(Ansi.BOLD + Ansi.BRIGHT_MAGENTA + "  [Goal]    " + Ansi.RESET
                        + Ansi.MAGENTA + "Track your intake and output to stay balanced.\n" + Ansi.RESET);
                break;
        }
