...
public class FileUtil {
    ...
    public static void readFromFile() throws IOException {
        ...
        for (List<String> rec : records) printDecoratedRecord(rec);
    }
    ...
    public static void readByName(String targetName) throws IOException {
        ...
        for (List<String> rec : matches) printDecoratedRecord(rec);
    }
    ...
    // ── Flexible decorated record printer ────────────────────────────────────

    private static void printDecoratedRecord(List<String> rec) {
        // Collect all display lines first so we can measure the widest one
        List<String[]> displayLines = new ArrayList<>(); // {text, color}

        for (String line : rec) {
            if (line.startsWith(RECORD_START)) continue;

            if (line.startsWith("====")) {
                // separator — handled as a divider row
        ...
        // ── Compute box inner width (based on visible text) ───────────────────
        int inner = 44; // minimum
        for (String[] dl : displayLines) {
            if (dl[0].equals("__DIVIDER__")) continue;
            // strip ANSI from the color prefix too
            int vlen = Ansi.visibleLen(dl[0]) + 4; // 4 = "  ║  " padding
        ...
        for (String[] dl : displayLines) {
            ...
            int spaces     = inner - 2 - textVlen; // 2 for leading spaces inside "║  "
